...
const CONFIG = {
  ...
  allProductsCover: "assets/backgrounds/belgery-product-board.png"
};
...
/* Laser animation restore from the uploaded animation draft.
   Keeps the current clean site, but forces the hero engraving sequence to run.
*/
(function(){
  function restartEngraving(){
    const hero = document.querySelector('.hero-engrave');
    const lockup = document.querySelector('.engrave-lockup');
    if (!hero || !lockup) return;
    hero.classList.remove('engraving-live');
    // force reflow so repeated visits/back-cache replay the animation
    void hero.offsetWidth;
    hero.classList.add('engraving-live');
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', restartEngraving);
  else restartEngraving();
  window.addEventListener('pageshow', restartEngraving);
})();

/* DIRECT FINAL PATCH — replay the engraved logo sequence from the files themselves. */
(function(){
  function replayBelgeryEngraving(){
    const hero = document.querySelector('.hero-engrave');
    const lockup = document.querySelector('.laser-engrave-lockup');
    if (!hero || !lockup) return;

    hero.classList.remove('engraving-live');
    lockup.querySelectorAll('.logo-reveal-mask,.logo-reveal,.logo-pixel-laser,.logo-burn-trail,.laser-line,.laser-haze,.engrave-smoke').forEach(el => {
  ...
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', replayBelgeryEngraving);
  else replayBelgeryEngraving();
  window.addEventListener('pageshow', replayBelgeryEngraving);
})();
