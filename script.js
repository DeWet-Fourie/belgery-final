...
function getImagesFromRow(row) {
  ...
  // Google Sheet gallery support: add up to 6 product photos with columns
  // image1, image2, image3, image4, image5, image6
  // Also accepts img1/photo1/picture1/productImage1 variants.
  for (let i = 1; i <= 6; i++) {
...
function productCard(product) {
  ...
  const imageCount = Math.min((product.images?.length || 1), 6);
  const galleryDots = imageCount > 1
    ? `<span class="product-photo-dots" aria-label="${imageCount} product photos">${Array.from({ length: imageCount }).map((_, index) => `<i class="${index === 0 ? "is-active" : ""}" aria-hidden="true"></i>`).join("")}</span>`
    : "";
...
(function(){
  function initCleanLogoIntro(){
    ...
    function render(){
      ...
      logo.style.transform = `translate3d(0, ${(26 - 30*p).toFixed(2)}px, 0) scale(${(0.965 + .035*p).toFixed(4)})`;

      if (header) {
        const nav = clamp((p - .12) / .44, 0, 1);
