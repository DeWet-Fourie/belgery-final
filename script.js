...
function productCard(product) {
  ...
  return `
    <article class="product-card" data-id="${escapeAttr(product.id)}" data-collection="${escapeAttr(collection)}">
      <button class="product-image" type="button" data-open-product="${escapeAttr(product.id)}" aria-label="Open ${escapeAttr(product.name)} product gallery">
        <img src="${escapeAttr(mainImage)}" alt="${escapeAttr(product.name)}" loading="lazy" onerror="this.src='${CONFIG.placeholderImage}'">
      </button>
      <div class="product-info">
        <div class="product-kicker"><span>${escapeHTML(collection)}</span><span>Handmade</span></div>
...
function renderFilters() {
  ...
  const hideAll = wrap.dataset.hideAll === "true";
  const collections = collectionList(allProducts);
  const visibleCollections = hideAll ? collections.filter(collection => collection !== "All") : collections;

  if (hideAll && activeCollection === "All" && visibleCollections.length) {
    activeCollection = decodeURIComponent(location.hash.replace("#", "")) || visibleCollections[0];
  }

  wrap.innerHTML = visibleCollections.map(collection => `
  ...
  $$(".filter-btn", wrap).forEach(button => {
    button.addEventListener("click", () => {
      ...
      history.replaceState(null, "", `#${encodeURIComponent(activeCollection)}`);
      ...
      const title = $("#activeCollectionTitle");
      if (title) title.textContent = activeCollection === "All" ? "All pieces" : `${activeCollection} pieces`;
  ...
  wrap.innerHTML = names.map(name => {
    ...
    return `
      <a class="collection-card" href="products.html#${encodeURIComponent(name)}" data-collection-card="${escapeAttr(name)}">
        ...
        <div>
          <p class="eyebrow">Shop collection</p>
          <h2>${escapeHTML(name)}</h2>
          <p>${items.length} piece${items.length === 1 ? "" : "s"} currently available.</p>
        </div>
      </a>
    `;
  }).join("");
...
function openProduct(id) {
  ...
  const priceHTML = hasSale
    ? `<div class="price-row"><span class="price-current">${escapeHTML(salePrice)}</span><span class="price-old">${escapeHTML(compareAtPrice)}</span></div><span class="sale-badge">Limited offer</span>`
    : `<p class="price">${escapeHTML(normalPrice || "Price on request")}</p>`;
...
async function init() {
  try {
    ...
    if ($("#productsGrid")) {
      ...
      const button = hashCollection
        ? $$("#collectionFilters .filter-btn").find(btn => btn.dataset.filter === hashCollection)
        : $$("#collectionFilters .filter-btn").find(btn => btn.classList.contains("active"));
...
document.addEventListener("click", event => {
  ...
  if (collectionCard && $("#collectionFilters")) {
    const collection = collectionCard.dataset.collectionCard;
    const button = $$("#collectionFilters .filter-btn").find(btn => btn.dataset.filter === collection);
    if (button) {
