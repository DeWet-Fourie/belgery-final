...
/* =========================================================
   FINAL PATCH — desktop collection click + All Products
   This intentionally overrides the earlier rail functions.
   ========================================================= */
(function(){
  const ALL_PRODUCTS_NAME = "All Products";

  function makeAllProductsCollection(productList, baseCollections){
    const first = baseCollections?.[0];
    return {
      name: ALL_PRODUCTS_NAME,
      slug: "all-products",
      ...
      cover: first?.cover || productList?.[0]?.images?.[0] || CONFIG.placeholderImage,
      storyCover: "",
  ...
  const originalBuildCollections = buildCollections;
  buildCollections = function patchedBuildCollections(productList){
    const base = originalBuildCollections(productList || []);
    return [makeAllProductsCollection(productList || [], base), ...base];
  };

  collectionRailCard = function patchedCollectionRailCard(collection, index){
    const isAll = collection.isAllProducts || collection.name === ALL_PRODUCTS_NAME;
    return `
      <button class="collection-rail-card ${isAll ? "all-products-card" : ""} ${collection.name === activeCollection ? "is-active" : ""}" type="button" data-collection="${escapeAttr(collection.name)}" style="--delay:${index * 40}ms">
        <div class="rail-wire-hanger" aria-hidden="true">
          <span class="rail-loop"></span>
          <span class="rail-wire"></span>
        </div>
        <span class="collection-count">${collection.products.length} piece${collection.products.length === 1 ? "" : "s"}</span>
        ${isAll ? `<div class="all-products-rail-art" aria-hidden="true"><span>ALL</span><span>PIECES</span></div>` : `<img src="${escapeAttr(collection.cover)}" alt="${escapeAttr(collection.name)} collection" loading="lazy" onerror="this.src='${CONFIG.placeholderImage}'">`}
        <span class="collection-rail-copy">
          <small>${escapeHTML(collection.tagline)}</small>
          <strong>${escapeHTML(collection.name)}</strong>
  ...
  function normaliseCollectionName(value){
    return clean(value).toLowerCase();
  }

  selectCollection = function patchedSelectCollection(name, options = {}){
    const wanted = normaliseCollectionName(name) || normaliseCollectionName(activeCollection) || normaliseCollectionName(collections[0]?.name);
    const collection = collections.find(item => normaliseCollectionName(item.name) === wanted) || collections[0];
    if (!collection) return;

    activeCollection = collection.name;

    $$("#collectionRail .collection-rail-card").forEach(card => {
      card.classList.toggle("is-active", normaliseCollectionName(card.dataset.collection) === normaliseCollectionName(collection.name));
    });
    ...
    const title = $("#productsTitle");
    if (title) title.textContent = collection.isAllProducts ? "All BELGERY products" : `${collection.name} products`;

    const whatsapp = $("#activeCollectionWhatsapp");
    if (whatsapp) {
      const message = collection.isAllProducts
        ? "Hi BELGERY, I would like to ask about your available products."
        : `Hi BELGERY, I would like to ask about the ${collection.name} collection.`;
      whatsapp.href = `https://wa.me/${CONFIG.whatsappNumber}?text=${encodeURIComponent(message)}`;
    }

    if (options.updateHash !== false) {
      history.replaceState(null, "", collection.isAllProducts ? "#all-products" : `#${encodeURIComponent(collection.name)}`);
    }
  ...
  renderMainCollectionRail = function patchedRenderMainCollectionRail(){
    const rail = $("#collectionRail");
    if (!rail) return;
    ...
    rail.innerHTML = collections.map(collectionRailCard).join("");
    const status = $("#collectionStatus");
    const realCollectionCount = Math.max(collections.length - 1, 0);
    if (status) status.textContent = `${realCollectionCount} collection${realCollectionCount === 1 ? "" : "s"} loaded + all products`;

    // Event delegation fixes desktop clicks even when cards are re-rendered or dragged.
    rail.onclick = event => {
  ...
  setupHorizontalRail = function patchedSetupHorizontalRail(rail){
    if (!rail) return;

    // Reset old listener guard from earlier builds.
    rail.dataset.dragReady = "final";
    rail.style.overflowX = "auto";
    rail.style.webkitOverflowScrolling = "touch";
    rail.style.scrollBehavior = "smooth";

    // Desktop drag is useful, but it must never swallow a normal click.
    let isDown = false;
    let startX = 0;
    let startLeft = 0;
    let dragged = false;
    ...
    rail.addEventListener("pointerdown", event => {
      ...
      isDown = true;
      dragged = false;
      rail.dataset.wasDragging = "false";
      startX = event.clientX;
      startLeft = rail.scrollLeft;
      rail.classList.add("is-dragging");
    });
    ...
    const endDrag = () => {
      ...
      isDown = false;
      rail.classList.remove("is-dragging");
      setTimeout(() => { rail.dataset.wasDragging = "false"; }, dragged ? 120 : 0);
    };
  ...
  initHashCollection = function patchedInitHashCollection(){
    const raw = decodeURIComponent(location.hash.replace("#", ""));
    if (!raw) return collections[0]?.name || ALL_PRODUCTS_NAME;
    if (raw.toLowerCase() === "all-products" || raw.toLowerCase() === "all products") return ALL_PRODUCTS_NAME;
    const match = collections.find(collection => collection.name.toLowerCase() === raw.toLowerCase());
    return match?.name || collections[0]?.name || ALL_PRODUCTS_NAME;
  };
})();
