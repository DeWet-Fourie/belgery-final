/* =========================================================
   BELGERY SHEET-ONLY CATALOGUE ENGINE — SAFE FULL FIELD VERSION
   Replace your entire script.js with this file.

   This version is defensive:
   - It works even if your Sheet headings have spaces/capital letters.
   - It supports shortDescription, longDescription, material/materials,
     dimensions, leadTime, image1-image12, gallery, salePrice,
     compareAtPrice, featured, available and WhatsApp messages.
   - It will still show products even if id is blank.
   ========================================================= */
...
async function fetchProducts() {
  ...
  console.log("BELGERY products loaded:", products);
  return products;
}
...
function parseCSV(text) {
  ...
  const headers = rawHeaders.map(header => normaliseKey(header));
...
function mapSheetRowToProduct(row, index) {
  ...
  const product = {
    ...
    colour: firstValue(row, ["colour", "color", "leatherColour", "leatherColor"]),
    finish: firstValue(row, ["finish", "hardware", "stitching"]),
    ...
    notes: firstValue(row, ["notes", "extraNotes", "note"])
  };

  product.images = getImagesFromRow(row);
  product.extraFields = getExtraFields(row);
...
function getExtraFields(row) {
  const used = new Set([
    "id","productid","sku","slug","name","productname","product","title",
    "collection","category","range","type","price","normalprice","regularprice","sellingprice",
    "saleprice","discountprice","specialprice","compareatprice","compareprice","oldprice","wasprice",
    "originalprice","previousprice","shortdescription","shortdesc","carddescription","summary",
    "descriptionshort","description","productdescription","maindescription","longdescription",
    "longdesc","fulldescription","details","story","material","materials","leather","fabric",
    "dimensions","dimension","size","measurements","leadtime","productiontime","deliverytime",
    "turnaround","colour","color","leathercolour","leathercolor","finish","hardware","stitching",
    "available","availability","status","featured","feature","homepage","home","sortorder",
    "sort","order","displayorder","whatsappmessage","whatsapp","enquirymessage","message",
    "notes","extranotes","note","gallery","images","imagegallery","photos","photogallery",
    "image","mainimage","coverimage","productimage","photo","picture"
  ]);
  ...
  return Object.entries(row)
    .filter(([key, value]) => clean(value) && !used.has(key))
    .map(([key, value]) => ({
...
function productCard(product) {
  ...
  const shortDescription =
    ...
    "Handmade leather piece from BELGERY.";
  ...
  return `
    <article class="product-card" data-id="${escapeAttr(product.id)}" data-collection="${escapeAttr(product.collection)}">
      ...
      <div class="product-info">
        ...
        <p class="short-description">${escapeHTML(shortDescription)}</p>
...
function openProduct(id) {
  ...
  const priceHTML = hasSale
    ? `
      <div class="price-row modal-price-row">
        <span class="price-current">${escapeHTML(product.salePrice)}</span>
        <span class="price-old">${escapeHTML(product.compareAtPrice)}</span>
      </div>
      <span class="sale-badge">Limited offer</span>
...
function productDetailsHTML(product) {
  ...
  const extra = (product.extraFields || []).filter(field => clean(field.value));

  const essentialHTML = essentials.length
    ? `
      <div class="modal-section">
  ...
  const notesHTML = clean(product.notes)
    ? `
      <div class="modal-section">
        <h3>Notes</h3>
  ...
  return essentialHTML + extraHTML + notesHTML;
}
