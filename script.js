...
function setupHorizontalRail(rail) {
  ...
  const end = () => {
    ...
    if (moved) setTimeout(() => { railDidDrag = false; }, 220);
  ...
  rail.addEventListener("pointermove", event => move(event.clientX, event.clientY, event));
  rail.addEventListener("pointerup", end);
  rail.addEventListener("pointercancel", end);
  rail.addEventListener("pointerleave", end);

  // iOS/WhatsApp in-app browser fallback: real touch-drag, non-passive.
