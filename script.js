...
function buildCollections(products) {
  ...
  const builtCollections = Array.from(map.values()).map(collection => ({
    ...collection,
    description: smartCollectionDescription(collection)
  }));

  // Special rail option: show every product in one place.
  // This is generated from the same Google Sheet, so the client does not need a separate collection.
  const allProductsCollection = {
    name: "All Products",
    slug: "all-products",
    tagline: "Complete BELGERY catalogue",
    description: "Browse every available BELGERY piece in one place, then open any product to view its full gallery and details.",
    cover: builtCollections[0]?.cover || products[0]?.images?.[0] || CONFIG.placeholderImage,
    storyCover: builtCollections[0]?.cover || products[0]?.images?.[0] || CONFIG.placeholderImage,
    products: products
  };

  return [allProductsCollection, ...builtCollections];
}
...
function renderMainCollectionRail() {
  ...
  // Use delegated pointer/click handling so rebuilt rail cards always open correctly.
  // The previous per-button click handler could be blocked by stale drag state in mobile browsers.
  if (rail.dataset.collectionClickReady !== "true") {
    rail.dataset.collectionClickReady = "true";
    let downX = 0;
    let downY = 0;
    let dragged = false;
    ...
    rail.addEventListener("pointermove", event => {
      if (Math.abs(event.clientX - downX) > 10 || Math.abs(event.clientY - downY) > 10) dragged = true;
    }, { passive: true });
    ...
    rail.addEventListener("click", event => {
      ...
      selectCollection(card.dataset.collection, { scroll: true, updateHash: true });
    });
  }
...
function selectCollection(name, options = {}) {
  ...
  if (title) title.textContent = collection.name === "All Products" ? "All products" : `${collection.name} products`;
...
function renderCollectionStory(collection) {
  ...
  const indexLabel = collection.name === "All Products"
    ? "ALL"
    : String(Math.max(1, collections.indexOf(collection))).padStart(2, "0");

  story.innerHTML = `
    <div class="story-index">${escapeHTML(indexLabel)}</div>
    ...
    <div class="story-main">
      ...
      <div class="story-meta-row">
        ...
        <span>${collection.name === "All Products" ? "Full catalogue" : "Curated collection"}</span>
...
function productCard(product) {
  ...
  const dots = product.images && product.images.length > 1
    ? `<div class="product-photo-dots" aria-label="${product.images.length} photos available">${product.images.slice(0, 6).map((_, index) => `<span class="${index === 0 ? "is-active" : ""}"></span>`).join("")}</div>`
    : "";
