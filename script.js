...
const CONFIG = {
  ...
  allProductsCover: "assets/backgrounds/dark-product-board.webp"
};
...
function sheetUrl() {
  ...
  return `${CONFIG.productsCsvUrl}${sep}cacheBust=${Math.floor(Date.now() / 300000)}`;
}
...
async function fetchProducts() {
  ...
  let res;
  try {
    res = await fetch(sheetUrl(), { cache: "default", signal: controller.signal });
  } finally {
...
function optimizeImage(src, width = 1400) {
  const v = clean(src);
  if (!v) return CONFIG.placeholderImage;
  if (!/res\.cloudinary\.com\/[^/]+\/image\/upload\//i.test(v)) return v;
  return v.replace(/(\/image\/upload\/)/, `$1f_auto,q_auto,dpr_auto,w_${width}/`);
}
...
function railCard(collection, index) {
  ...
  return `
    <button class="collection-rail-card ${isAll ? "all-products-card" : ""} ${collection.name === activeCollection ? "is-active" : ""}" type="button" data-collection="${escapeAttr(collection.name)}" style="--delay:${index * 35}ms">
      ...
      <img src="${escapeAttr(optimizeImage(collection.cover, 800))}" alt="${escapeAttr(collection.name)} collection" loading="lazy" onerror="this.src='${CONFIG.placeholderImage}'">
  ...
  return `
    <article class="product-card" data-id="${escapeAttr(product.id)}">
      <button class="product-image" type="button" data-open-product="${escapeAttr(product.id)}" aria-label="Open ${escapeAttr(product.name)} gallery">
        <img src="${escapeAttr(optimizeImage(img, 700))}" alt="${escapeAttr(product.name)}" loading="lazy" decoding="async" onerror="this.onerror=null;this.src='${CONFIG.placeholderImage}'">
        ${photoDots(product)}
      </button>
...
function openProduct(id) {
    ...
    <div class="modal-inner upgraded-product-modal">
      ...
      <section class="modal-visual-panel" aria-label="${escapeAttr(product.name)} image gallery">
        <div class="modal-gallery">
          ${images.map((src, index) => `<figure class="modal-photo"><img ${index === 0 ? `src="${escapeAttr(optimizeImage(src, 1600))}" fetchpriority="high"` : `data-src="${escapeAttr(optimizeImage(src, 1600))}"`} alt="${escapeAttr(product.name)} photo ${index + 1}" loading="${index === 0 ? 'eager' : 'lazy'}" decoding="async" onerror="this.onerror=null;this.src='${CONFIG.placeholderImage}'"></figure>`).join("")}
        </div>
        ${images.length > 1 ? `<div class="modal-thumb-strip" aria-label="${images.length} product photos">${images.map((src, index) => `<span class="modal-thumb" data-index="${index}"><img ${index === 0 ? `src="${escapeAttr(optimizeImage(src, 180))}"` : `data-src="${escapeAttr(optimizeImage(src, 180))}"`} alt="${escapeAttr(product.name)} thumbnail ${index + 1}" loading="lazy" decoding="async" onerror="this.onerror=null;this.src='${CONFIG.placeholderImage}'"></span>`).join("")}</div>` : ""}
      </section>
  ...
  requestAnimationFrame(() => {
    ...
    const loadIndex = (index) => {
      const photo = gallery?.querySelectorAll('.modal-photo')[index];
      loadImage(photo?.querySelector('img'));
      const thumb = modal.querySelectorAll('.modal-thumb')[index];
      loadImage(thumb?.querySelector('img'));
      const next = gallery?.querySelectorAll('.modal-photo')[index + 1];
      loadImage(next?.querySelector('img'));
    };
    loadIndex(0);
    ...
    gallery?.addEventListener('scroll', () => {
      const index = Math.max(0, Math.min(images.length - 1, Math.round(gallery.scrollLeft / Math.max(1, gallery.clientWidth))));
      loadIndex(index);
      modal.querySelectorAll('.modal-thumb').forEach((t, i) => t.classList.toggle('is-current', i === index));
    }, {passive:true});
  });
}

function detailsHTML(product) { return ""; }
