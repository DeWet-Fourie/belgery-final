...
(function(){
  function initBagIntro(){
    ...
    function render(){
      ...
      const leftMove = isMobile ? -96 : isTablet ? -82 : -72;
      const rightMove = isMobile ? 112 : isTablet ? 94 : 82;
      const dropLeft = isMobile ? 18 + (16 * p) : 18 + (12 * p);
      const dropRight = isMobile ? 22 + (18 * p) : 22 + (14 * p);

      if (isMobile) {
        left.style.transform = `translate3d(calc(-50% + ${leftMove * p}vw), ${dropLeft}px, 0) rotate(${-2 + (-22 * p)}deg)`;
        right.style.transform = `translate3d(calc(-50% + ${rightMove * p}vw), ${dropRight}px, 0) rotate(${2 + (22 * p)}deg)`;
      } else {
        left.style.transform = `translate3d(${leftMove * p}vw, ${dropLeft}px, 0) rotate(${-2 + (-22 * p)}deg)`;
        right.style.transform = `translate3d(${rightMove * p}vw, ${dropRight}px, 0) rotate(${2 + (22 * p)}deg)`;
      }

      const reveal = clamp((p - 0.14) / 0.46, 0, 1);
      ...
      if (header) {
        const headerReveal = clamp((p - 0.18) / 0.42, 0, 1);
        header.style.opacity = headerReveal.toFixed(3);
        header.style.transform = `translate3d(-50%, ${(-18 * (1 - headerReveal)).toFixed(2)}px, 0)`;
        header.style.pointerEvents = headerReveal > 0.92 ? 'auto' : 'none';
        header.classList.toggle('header-revealed', headerReveal > 0.92);
      }

      if (Math.abs(target - current) > 0.001) requestAnimationFrame(render);
      else { current = target; running = false; }
    }
