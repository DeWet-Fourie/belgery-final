...
const SYSTEM_FIELD_KEYS = new Set([
  "id", "name", "collection", "price", "salePrice", "compareAtPrice",
  "shortDescription", "description", "longDescription", "whatsappMessage",
  "available", "featured", "sortOrder", "gallery",
...
function productDetailRows(product) {
  const preferredKeys = [
    "material", "dimensions", "leadTime", "colour", "color", "size",
    "hardware", "finish", "strapLength", "care", "includes", "notes", "madeToOrder"
  ];
  ...
  preferredKeys.forEach(key => {
    ...
    if (value && !used.has(key)) {
      rows.push([labelFromKey(key), value]);
      used.add(key);
    }
  });

  Object.keys(product).forEach(key => {
    if (used.has(key) || SYSTEM_FIELD_KEYS.has(key)) return;
    const value = clean(product[key]);
    if (!value) return;
    rows.push([labelFromKey(key), value]);
  });
...
function productCard(product) {
  ...
  const normalPrice = clean(product.price);
  const salePrice = clean(product.salePrice);
  const compareAtPrice = clean(product.compareAtPrice);
  const hasSale = salePrice && compareAtPrice;

  const priceHTML = hasSale
    ? `<div class="price-row"><span class="price-current">${escapeHTML(salePrice)}</span><span class="price-old">${escapeHTML(compareAtPrice)}</span></div><span class="sale-badge">Limited offer</span>`
    : `<div class="price-row"><span class="price-current">${escapeHTML(normalPrice || "Price on request")}</span></div>`;
    ...
    <article class="product-card" data-id="${escapeAttr(product.id)}" data-collection="${escapeAttr(collection)}">
      ...
      <div class="product-info">
        ...
        <div class="product-actions">
          <button class="small-btn" type="button" data-open-product="${escapeAttr(product.id)}">View gallery</button>
...
function openProduct(id) {
  ...
  const images = getImages(product);
  const galleryImages = images.length ? images : [CONFIG.placeholderImage];

  const shortDescription = clean(product.shortDescription);
  const description = clean(product.description);
  const longDescription = clean(product.longDescription);
  const overview = longDescription || description || shortDescription || "Contact BELGERY for more details about this piece.";
  const extraOverview = description && longDescription && description !== longDescription ? description : "";
  ...
  const priceHTML = hasSale
    ? `<div class="price-row modal-price"><span class="price-current">${escapeHTML(salePrice)}</span><span class="price-old">${escapeHTML(compareAtPrice)}</span></div><span class="sale-badge">Limited offer</span>`
    : `<p class="price modal-price-single">${escapeHTML(normalPrice || "Price on request")}</p>`;

  const detailRows = productDetailRows(product);

  modal.innerHTML = `
    <div class="modal-inner product-interface">
      ...
      <div class="modal-gallery" aria-label="${escapeAttr(product.name)} image gallery">
        ${galleryImages.map((img, index) => `
          <figure class="modal-slide">
            <img src="${escapeAttr(img)}" alt="${escapeAttr(product.name)} image ${index + 1}" onerror="this.src='${CONFIG.placeholderImage}'">
          </figure>
        `).join("")}
      </div>

      <div class="modal-copy">
        <div class="modal-copy-inner">
          ...
          <section class="product-overview">
            ...
            ${extraOverview ? `<p>${escapeHTML(extraOverview)}</p>` : ""}
          </section>

          ${renderDetailRows(detailRows)}

          <section class="product-enquiry-panel">
            <h3>Enquire about this piece</h3>
            <p>Ask about availability, colour options, sizing or custom changes directly on WhatsApp.</p>
