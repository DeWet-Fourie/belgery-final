...
function railCard(collection, index) {
  ...
  return `
    <button class="collection-rail-card ${isAll ? "all-products-card" : ""} ${collection.name === activeCollection ? "is-active" : ""}" type="button" data-collection="${escapeAttr(collection.name)}" style="--delay:${index * 35}ms">
      ...
      ${isAll ? `<span class="all-products-rail-art" aria-hidden="true"></span>` : `<img src="${escapeAttr(collection.cover)}" alt="${escapeAttr(collection.name)} collection" loading="lazy" onerror="this.src='${CONFIG.placeholderImage}'">`}
...
function productCard(product, collection) {
  ...
  const note = clean(product.shortDescription) || (!collection?.isAllProducts ? clean(collection?.description) : "");
  return `
...
function openProduct(id) {
  ...
  const description = clean(product.longDescription) || clean(product.description) || clean(product.shortDescription) || "Contact BELGERY for more details about this piece.";
  modal.innerHTML = `
    <div class="modal-inner">
      <button class="small-btn close-btn" type="button" onclick="document.getElementById('productModal').close()">Close</button>
      <div class="modal-gallery">${images.map(src => `<img src="${escapeAttr(src)}" alt="${escapeAttr(product.name)}" onerror="this.src='${CONFIG.placeholderImage}'">`).join("")}</div>
      <div class="modal-copy">
        ...
        ${detailsHTML(product)}
        <a class="btn primary" target="_blank" rel="noopener" href="${whatsappLink(product)}">Enquire on WhatsApp</a>
      </div>
    </div>`;
  modal.showModal();
}
...
function detailsHTML(product) {
  const details = [["Material", product.material], ["Dimensions", product.dimensions], ["Lead time", product.leadTime]].filter(([,v]) => clean(v));
  if (!details.length) return "";
  return `<div class="modal-details">${details.map(([k,v]) => `<div><strong>${escapeHTML(k)}</strong><span>${escapeHTML(v)}</span></div>`).join("")}</div>`;
}
